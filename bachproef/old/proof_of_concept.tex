...
Om de theoretische concepten rond onderzoeksautonomie en de nauwe verweving van code en documentatie te valideren, is een geïntegreerd systeem ontwikkeld als Proof of Concept (PoC). Dit systeem fungeert als een georkestreerd ecosysteem waarbij de robuuste \text{.NET}-backend van ILVO de regie voert over interactieve Jupyter-omgevingen. De kern van de oplossing ligt in een hybride, 'switchable' architectuur: een opzet die het mogelijk maakt om op granulair niveau te wisselen tussen de officieel gecompileerde C\#-implementatie en een interpretatieve, door de onderzoeker aangestuurde Python-omgeving. Hiermee wordt een dynamische samenwerkingsvorm gerealiseerd waarbij de stabiliteit van de backend gewaarborgd blijft, terwijl de onderzoeker de vrijheid krijgt om direct op de actuele rekenmodellen te experimenteren. Dit hoofdstuk biedt een diepgaand overzicht van de infrastructuur, de architecturale keuzes en de technische realisatie van deze proefopstelling.

\section{Infrastructuur: docker-orchestratie}
...
{\sloppy Een cruciaal onderdeel van de implementatie is de platformonafhankelijke communicatielaag. De applicatie detecteert op welk besturingssysteem zij draait en past de verbindingsmethode hierop aan. Op Windows-ontwikkelmachines wordt gebruikgemaakt van named pipes (\nolinkurl{npipe://./pipe/docker\_engine}), terwijl op de Linux-gebaseerde productieservers van ILVO rechtstreeks via Unix-sockets (\nolinkurl{unix:///var/run/docker.sock}) wordt gecommuniceerd. De backend zorgt niet alleen voor het aanmaken van de container, maar configureert ook de noodzakelijke netwerkisolatie en resource-limieten.\par}

\subsection{Log-parsing en dynamische token-extractie}
Wanneer een Jupyter-container opstart, genereert deze automatisch een unieke beveiligingstoken voor authenticatie. {\sloppy Om de ervaring voor de onderzoeker zo drempelloos mogelijk te maken, wordt deze token automatisch geëxtraheerd. De C\#-backend monitort de log-stream van de container. \par}
...
De container fungeert als het laboratorium van de onderzoeker. De inrichting ervan is cruciaal om de technische barrière tussen de gecompileerde .NET-wereld en de interactieve Python-wereld weg te nemen. Voordat de backend een container kan instantiëren, moet de bijbehorende Docker-image beschikbaar zijn in de lokale image-registry. Deze ``jupyter-image'' wordt gebouwd met het commando \texttt{docker build -t jupyter-image .}, waarbij de Dockerfile uit Bijlage \ref{lst:dockerfile} als blauwdruk dient. Dit build-proces zorgt voor de pre-installatie van alle afhankelijkheden en het inbedden van de verzamelde DLL's, waardoor de opstarttijd van individuele containers minimaal blijft en de reproduceerbaarheid wordt gegarandeerd.

\subsection{De .NET 10.0 Runtime en Python-omgeving}
De Docker-image is opgebouwd rond de \nolinkurl{mcr.microsoft.com/dotnet/runtime:10.0} base-image van Microsoft. Door de officiële .NET runtime als basis te nemen, is native ondersteuning voor de C\#-DLL's gegarandeerd. Bovenop deze image worden Python 3 en de noodzakelijke pakketten zoals \texttt{jupyterlab} en \texttt{pythonnet} geïnstalleerd.

\subsection{Automatische Initialisatie via IPython}
Om te voorkomen dat onderzoekers bij het begin van elke notebook een lijst van complexe import-commando's moeten schrijven, wordt gebruikgemaakt van het IPython-configuratiemechanisme. Dit startup-script (zie codefragment \ref{lst:startuppy} in Bijlage \ref{ch:codefragmenten}) zorgt ervoor dat bij elke nieuwe sessie automatisch de CLR wordt geladen en alle relevante namespaces worden geïmporteerd. Hierdoor kan een onderzoeker direct in de notebook types zoals \texttt{Rekenfactor} gebruiken, alsof het native Python-klassen zijn.
...
{\sloppy In de PoC is dit principe gedemonstreerd aan de hand van de \nolinkurl{CustomerListTransformerFactory}. De factory fungeert als de beslissingslaag die de orchestrator ontzorgt. \par}
...
De onderzoeker kan vervolgens een script aanleveren (bijv. \nolinkurl{correctie\_emissie.py}) dat direct door de professionele C\#-omgeving wordt uitgevoerd:
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{poc_architectuur}
    \caption{Architecturaal overzicht van de Proof of Concept met C\#-orchestratie en hybride Python-integratie.}
    \label{fig:poc_architectuur}
\end{figure}

\section{Resultaten en Beperkingen}

\textit{Nota betreffende gegevensbescherming: Vanwege de strikte vertrouwelijkheid en het databeleid van ILVO is voor de visualisaties en resultaten in dit hoofdstuk gebruikgemaakt van synthetische datasets. Deze data simuleren de functionele werking van de originele rekenmodellen op een representatieve wijze, zonder bedrijfsgevoelige informatie of intellectueel eigendom te onthullen.}

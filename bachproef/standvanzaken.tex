...
\textcite{IronPython} is een open source implementatie van de Python programmeer taal dat is geintegreerd met .NET.
IronPython kan zoals pythonnet .NET dll's aanspreken via `clr` en kan andere python code ook gebruiken.
...
heeft IronPython support voor versie 2.7 en 3.4.
...
Aangezien dat we dll's nodig gaan hebben en mogelijks c\# code gaan hebben, starten we in de Dockerfile vanuit een Microsoft runtime docker image.
...
Deze dockerfile downloads de nodige packages, kopieert de dlls in de dlls folder en de notebooks uit de notebook folder en start de jupyter notebook server.

De startup.py is een python bestand dat bij het maken van de docker image code uitvoert tijdens dat de kernel start. Hierdoor kunnen er dll's geladen worden voordat de gebruiker toegang heeft en veel boilerplate mag weglaten.

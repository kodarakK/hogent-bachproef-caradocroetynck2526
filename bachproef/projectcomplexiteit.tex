...
Hoewel de overstap naar een modern webframework (.NET Razor Pages) veel van de technologische beperkingen van het oude Windows Forms-project wegneemt, blijft EMAV-web een applicatie met een aanzienlijke cognitieve last voor ontwikkelaars. In dit hoofdstuk wordt de gelaagde complexiteit van het project geanalyseerd, variërend van de wetenschappelijke domeinlogica tot de technische infrastructuur.

Achtereenvolgens wordt ingegaan op de kloof tussen wetenschappelijke modellen en software-implementatie, de uitdagingen van hybride databeheer en de problematiek van de ``Legacy Bridge'' bij het hergebruiken van oude rekenmodules. Tevens worden de infrastructurele implicaties van container-orchestratie besproken. Deze inventarisatie van uitdagingen vormt het noodzakelijke kader om te begrijpen waarom de architectuur van EMAV-web steunt op een uitgebreid frame van software-ontwerppatronen (design patterns) en de principes van Clean Architecture, die in de tweede helft van dit hoofdstuk in detail worden uitgediept.
...
In tegenstelling tot klassieke administratieve applicaties vormt de kern van EMAV een web van onderling afhankelijke wetenschappelijke formules die gebaseerd zijn op jarenlang landbouwkundig onderzoek.
...
Deze aanpak voorkomt het ontstaan van Gouden Klassen die alle logica van een bepaald domein bevatten, in plaats van dat de logica gedistribueerd is over honderden kleine, gefocuste klassen die elk één specifieke taak hebben. 
...
\section{Besluit: Een synergetische oplossing voor gelaagde complexiteit}
De diepgaande analyse van de diverse complexiteitsfactoren binnen EMAV-web — van de wetenschappelijke domeinlogica tot de infrastructurele orkestratie — toont aan dat een enkelvoudige technische ingreep onvoldoende zou zijn. Elk van de besproken design patterns vormt een essentiële stap om een specifieke barrière te doorbreken: de \textit{Repositories} temmen de hybride datastructuur, \textit{CQRS} en de \textit{Mediator} verlagen de cognitieve last, \textit{Strategieën} bieden wetenschappelijke flexibiliteit en \textit{Adapters} slaan de brug naar de waardevolle legacy-kern.

Het is echter de bewuste synergie tussen deze patronen en de overkoepelende \textit{Clean Architecture} die de werkelijke oplossing vormt. Door de businesslogica strikt te isoleren via deze patronen, wordt een systeem gecreëerd dat niet alleen technisch robuust is voor IT-ontwikkelaars, maar dat ook de noodzakelijke transparantie biedt om wetenschappelijke modellen voor onderzoekers te ontsluiten. Deze gelaagde architecturale aanpak is de meest solide en duurzame keuze voor het project; het vormt het onmisbare fundament voor de hybride integratiestrategie die in de volgende hoofdstukken wordt gevalideerd via de Proof of Concept.
